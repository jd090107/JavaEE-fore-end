...
export default {
  ...
  methods: {
    ...
    start() {
      // 开始测试,向后端请求
      let _self=this;
      _self.axios.post('/workload',_self.qs.stringify({clients:_self.test.users,hatchRate:_self.test.hatchRate,host:_self.test.host,runTime:_self.test.runTime,scenarioId:0,account:_self.account,taskName:_self.test.taskName})).then(function (res) {
